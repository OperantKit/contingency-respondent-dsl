...
このリポジトリは本設計チェックポイント時点で公開リリースチャネルを
もたない。したがって、本パッケージは**バージョンを宣言しない**
（「v1」なし、「メジャーバンプ」なし、「Core 凍結」なし）。設計変更は
[`auxiliary.md`](auxiliary.md) の中でチェックポイントとして記録される
— 加算的、時系列付きのエントリ、セマンティックバージョン演算は
行わない。

これは Core の規律（`contingency-dsl/spec/en/auxiliary.md` 参照）を
反映しており、リモート未設定でバージョン宣言が意味を持たないという
Ψ 計画の判断を受け継いでいる。
